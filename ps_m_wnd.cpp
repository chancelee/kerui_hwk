...
void PS_M_Wnd::slot_disconn(Client_Session *p_cs)
{
    ...
    //停止接收 关闭socket移除数据
    this->m_p_ps_sock->cs_remove(p_cs->m_sock);
    //delete p_cs;
}
...
void PS_M_Wnd::slot_create_dlg(Client_Session *p_cs, int n_type)
{
    ...
    case PSC_CMD_REPLY:
    {
        ...
        {
            p_cs->m_cmd_dlg.reset(new Cmd_Dlg(p_cs->m_sock));
        }
        p_cs->m_cmd_dlg->show();

        //发送一个回复包 表示窗口已经就位 让被控端开启线程
        Data_Head data_head(PSC_CMD, PSC_CMD_REPLY);
        Net_Tools::send_buf(p_cs->m_sock,
                            (char*)&data_head,
                            sizeof(Data_Head));
    ...
    case PSC_SCR_REPLY:
    {
        ...
        {
            p_cs->m_scr_dlg.reset(new Scr_Dlg(p_cs->m_sock));
        }
        p_cs->m_scr_dlg->show();
